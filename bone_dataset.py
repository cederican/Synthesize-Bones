        ...
        for _, idx in enumerate(data_lst):
            ...
            for mask_group_idx, mask_group in enumerate(self.mask_group_list):
                ...
                if self.top_only:
                    ...
                    range_z = (mask.shape[3] - mask.shape[3] // 2, mask.shape[3] - mask.shape[3] // 8)
                    mask = mask[:, range_x[0] : range_x[1], range_y[0] : range_y[1], range_z[0] : range_z[1]]
                
                
                np.save(os.path.join(os.path.join(self.output_path, idx), group_name + ".npy"), mask)
...
if __name__ == "__main__":
    dc = DatasetCreator(overfit=80, top_only=True)
    dc.create_dataset()
    print("Done!")
