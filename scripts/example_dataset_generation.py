...
if __name__ == "__main__":
    ...
    for _, idx in enumerate(data_lst):
            if not os.path.isdir(os.path.join(input_path, idx)):
                continue
            mesh_files = [os.path.join(input_path, idx, f) for f in os.listdir(os.path.join(input_path, idx)) if f.endswith('.obj')]

            for mesh_file in mesh_files:
                output_dir = os.path.join(input_path, idx, mesh_file.split('/')[-1].split('.')[0])
                ...
                fracture.generate_fractures(mesh_file,
                                            ...
                                            impact_intervall=[0.3, 1.0], # -0.5, 0.5
                                            threshold_interval=[100, 200]) # 0.0, 1000.0